...
const OptionSelector = ({ options, setOptions }: Props) => {
  ...
  return (
    <div>
      <h3 className="text-lg font-medium mb-2">2. オプションを選択</h3>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {optionList.map(opt => (
          <div key={opt.id} className="flex items-center p-4 border rounded-lg cursor-pointer hover:bg-gray-50" onClick={() => handleOptionChange(opt.id as keyof Options)}>
            <input 
              type="checkbox" 
